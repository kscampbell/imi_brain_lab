...
```
```{r gamma distribution model - DONT RUN}
...
```
 ...
 1 0     June 26  63.1   63.1  2.15  59.6   66.6
 2 0     July 17  69.7   69.7  2.3   66.1   73.5
 
 3 0.1   June 26  57     57.0  1.98  53.9   60.2
 4 0.1   July 17  69.3   69.2  2.43  65.5   73.3
 
 5 1     June 26  60.8   60.8  2.11  57.5   64.2
 6 1     July 17  67.4   67.3  2.31  63.8   71.2
 
 7 5     June 26  62.4   62.4  2.13  59.0   65.9
 8 5     July 17  69.9   69.8  2.37  66.1   73.7
 
 9 10    June 26  59.0   59.0  2.07  55.8   62.4
10 10    July 17  64.7   64.7  2.28  61.2   68.4
...
ggplot(lab, aes(x=IMI_ng_mg_protein, y=Mass_diff)) + geom_point(aes(color=Treatment)) + geom_smooth(method="lm") + scale_y_log10()
```

```{r}
lab_long_mass <- lab %>% 
  gather(key = "Mass_date", value = "Body_mass_g", -Animal_ID, -Treatment, -June_26, -July_17, -IMI_ng_g_tissue, -IMI_ole_ng_g_tissue, -IMI_ng_mg_protein, -IMI_ole_ng_mg_protein, -CRKT_sec_July_4, -CRKT_sec_July_8, -CRKT_sec_July_11, -TL_diff)

# reorder levels
lab_long_mass$Mass_date <- factor(lab_long_mass$Mass_date, levels = c("Mass_g_June_26", "Mass_g_July_3", "Mass_g_July_10", "Mass_g_July_17"))


ggplot(lab_long_mass, aes(x=Treatment, y=Body_mass_g)) + geom_boxplot() + geom_jitter(aes(color=Treatment)) + facet_wrap(~Mass_date)
```
